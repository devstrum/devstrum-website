    ...
    <div style={{ background: bg, color: ink, fontFamily: '"Geist Mono", monospace', minHeight: '100%' }} data-screen-label="Home">
        ...
        <div style={{ maxWidth: 900 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 40, flexWrap: 'wrap' }}>
            <a href={`mailto:${CONTACT_EMAIL}`} onClick={copyEmail} style={{ textDecoration: 'none' }}>
              <button style={{ padding: '14px 26px', background: ink, color: bg, border: 0, fontSize: 12, fontWeight: 600, letterSpacing: '.18em', fontFamily: 'inherit', cursor: 'pointer' }}>{copied ? 'EMAIL COPIED ✓' : 'BOOK A FREE AUDIT →'}</button>
            </a>
      ...
      <section id="work" style={{ padding: `0 ${PAD_X}`, borderTop: `1px solid ${ink}`, borderBottom: `1px solid ${ink}` }}>
        ...
        {[
          ...
          <a key={n} href="/work" className="ds-card-link" style={{ textDecoration: 'none', color: 'inherit' }}>
            {isMobile ? (
      ...
      <section style={{ padding: `clamp(48px, 10vw, 96px) ${PAD_X}`, borderTop: `1px solid ${ink}` }}>
        ...
        <h2 style={{ fontFamily: '"Geist", sans-serif', fontSize: 'clamp(32px, 6vw, 56px)', fontWeight: 600, letterSpacing: '-0.04em', margin: '0 0 56px', maxWidth: 760 }}>Senior builders. Real outcomes. No lock-in.</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 1, background: rule, border: `1px solid ${rule}` }}>
          {[
      ...
      <SiteFooter heading="Let's build your" headingAccent="automation." />
    </div>
  );
};
